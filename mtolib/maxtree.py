...
__suffix__ = importlib.machinery.EXTENSION_SUFFIXES[0]
...
class MaxTree:
    ...
    def ctypes_maxtree(self, params):
        # Create image object
        img_pointer = self.image.ravel().ctypes.data_as(
            ct.POINTER(params.d_type)
        )
        ...
        return mt_class.MtData(
            ...
            img=mt_class.Image(
                img_pointer, *self.image.shape, self.image.size
            ),
...
class OriginalMaxTree(MaxTree):
    def __init__(self, image, verbosity, params):
        ...
        if params.d_type == ct.c_double:
            self.mt_lib = ct.CDLL(
                os.path.join(
                    ...
                    "lib",
                    "maxtree_double" + __suffix__,
                )
            )
        else:
            self.mt_lib = ct.CDLL(
                os.path.join(
                    os.path.dirname(__file__), "lib", "maxtree" + __suffix__
                )
            )
        ...
        c_img = mt_class.Image(
            img_pointer, image.shape[0], image.shape[1], image.size
        )
